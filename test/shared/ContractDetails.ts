...
export class ContractDetails {
  ...
  constructor(_name?: string, _symbol?: string, _totalSupply?: string, _maxMintedSupply?: string, _decimals?: string) {
    const name = _name ?? process.env.CONTRACT_NAME;
    if (!name) {
      throw new Error('Unset CONTRACT_NAME');
    }
    const symbol = _symbol ?? process.env.CONTRACT_SYMBOL;
    if (!symbol) {
      throw new Error('Unset CONTRACT_SYMBOL');
    }
    const totalSupply = _totalSupply ?? process.env.CONTRACT_TOTAL_SUPPLY_IN_WEI;
    if (!totalSupply) {
      throw new Error('Unset CONTRACT_TOTAL_SUPPLY');
    }
    const decimals = _decimals ?? process.env.CONTRACT_DECIMALS;
    if (!decimals) {
      throw new Error('Unset CONTRACT_DECIMALS');
    }
    const maxMintedSupply = _maxMintedSupply ?? process.env.CONTRACT_MAX_MINTED_AMOUNT_IN_WEI;
    if (!maxMintedSupply) {
      throw new Error('Unset CONTRACT_MAX_MINTED_AMOUNT');
    }
    this.name = name;
